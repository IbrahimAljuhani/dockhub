...
backup_redmine() {
    ...
    if docker exec redmine-db pg_dump -U "$pg_user" "$pg_db" > "$dump_file" 2>"$_derr"; then
...
restore_redmine() {
    ...
    if [[ -f "$install_dir/db.sql" ]]; then
        ...
        _r=0; for _i in $(seq 1 30); do
            docker exec "redmine-db" pg_isready -h localhost -U "$pg_user" -d "$pg_db" >/dev/null 2>&1 && { _r=1; break; }
            sleep 2
        done
        if (( ! _r )); then
            print_warn "redmine-db not ready in 60s. Volume restore stands; dump NOT replayed."
        ...
        _e="$(docker exec "redmine-db" dropdb --force --if-exists -U "$pg_user" "$pg_db" 2>&1)" \
          && _e="$(docker exec "redmine-db" createdb -U "$pg_user" -O "$pg_user" "$pg_db" 2>&1)" \
          || { print_warn "Could not recreate $pg_db: ${_e:-unknown}"
               print_warn "SKIPPING the replay — the restored volume is intact and usable."
               rm -f "$install_dir/db.sql"; return 0; }
        if docker exec -i "redmine-db" psql -v ON_ERROR_STOP=1 --single-transaction -q -o /dev/null \
