...
backup_nextcloud() {
    ...
    if docker exec nextcloud-db pg_dump -U nextcloud nextcloud > "$dump_file" 2>"$_derr"; then
...
restore_nextcloud() {
    ...
    if [[ -f "$install_dir/db.sql" ]]; then
        ...
        _r=0; for _i in $(seq 1 30); do
            docker exec "nextcloud-db" pg_isready -h localhost -U "nextcloud" -d "nextcloud" >/dev/null 2>&1 && { _r=1; break; }
            sleep 2
        done
        if (( ! _r )); then
            print_warn "nextcloud-db not ready in 60s. Volume restore stands; dump NOT replayed."
        ...
        _e="$(docker exec "nextcloud-db" dropdb --force --if-exists -U "nextcloud" "nextcloud" 2>&1)" \
          && _e="$(docker exec "nextcloud-db" createdb -U "nextcloud" -O "nextcloud" "nextcloud" 2>&1)" \
          || { print_warn "Could not recreate nextcloud: ${_e:-unknown}"
               print_warn "SKIPPING the replay — the restored volume is intact and usable."
               rm -f "$install_dir/db.sql"; return 0; }
        if docker exec -i "nextcloud-db" psql -v ON_ERROR_STOP=1 --single-transaction -q -o /dev/null \
               -U "nextcloud" -d "nextcloud" < "$install_dir/db.sql"; then
            print_info "Database restored from db.sql (dropped, recreated, replayed in one transaction)."
        else
            print_warn "db.sql failed to replay and was rolled back — nextcloud is now EMPTY."
        fi
