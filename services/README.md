...
![Progress](https://img.shields.io/badge/built-42%20%2F%2042%20services-46a049?style=for-the-badge)

[`services.sh`](services.sh) presents these grouped by category. **Everything listed below is built and deployable** — that is why nothing carries a status marker: a tick on all 42 rows tells you nothing that this sentence does not.

The mechanism behind it still exists for whatever gets added next. A service that appears in `services.sh`'s `CATALOG` without a `deploy.sh` shows `(coming soon)` when picked, and starts working the moment that file lands — there is no flag to flip.
...
>
> Icons come from [dashboard-icons](https://github.com/homarr-labs/dashboard-icons) over jsDelivr, a CDN published for exactly this. **Nothing here hot-links a project's own web server.** Two rows used to — one of them, LocalAI's, had already gone dead — and a project that moves a file breaks this page without ever knowing this page exists. Where the CDN has no icon, use a plain shields.io badge rather than pointing at someone's origin.

| Category | Services |
|---|---|
| **[AI](AI/)** | [![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat-square&logo=ollama&logoColor=white)](AI/ollama/) · <a href="AI/open-webui/"><img src="https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/open-webui.svg" width="20" height="20" alt="Open WebUI" title="Open WebUI"></a> · [![llama.cpp](https://img.shields.io/badge/llama.cpp-303030?style=flat-square)](AI/llama-cpp/) · [![LocalAI](https://img.shields.io/badge/LocalAI-303030?style=flat-square)](AI/localai/) |
...
| **[Security](Security/)** | [![Vaultwarden](https://img.shields.io/badge/Vaultwarden-000000?style=flat-square&logo=vaultwarden&logoColor=white)](Security/vaultwarden/) · [![SearXNG](https://img.shields.io/badge/SearXNG-3050FF?style=flat-square&logo=searxng&logoColor=white)](Security/searxng/) |
...
The category/service list itself lives in [`services.sh`](services.sh)'s `CATALOG` array — this table mirrors it and is not the source of truth.

⚠️ **Three things here are maintained by hand and will drift if you forget one:** the count badge above, this table, and the port list below. `CATALOG` and the `deploy.sh` files are what the menu actually reads.
...
| Langflow | `7860` (optional). ⚠️ Its **Custom Component** nodes run Python inside the container, so `deploy.sh` asks about `main-net` before it asks about the port — see the [category threat model](Multi-Agent/README.md). Its Postgres has no host port at all, where upstream's compose publishes `5432`. |
| Flowise | `3200` (optional) → the container's `3000`, which is Open WebUI's, Redmine's and Juice Shop's default and is why it is not offered here. Same Custom Tool warning as Langflow, in JavaScript. |
| Dify | `8088` (optional) → the nginx container's `80`. Upstream publishes **80 and 443** unconditionally, which would fight NPM and take the host's HTTP down; both are removed. Its plugin debugging port `5003` is closed too, and `deploy.sh` refuses to start if any `:5003` binding reappears. |
| Paperclip | `3100` (optional). Off `main-net` the port is the only way in, so it is required rather than offered. |
| Redmine | `3000` |
...
3. If `<slug>` isn't already in `services.sh`'s `CATALOG` array as a `🚧` placeholder, add it there too (`Category|slug|Display Name`) — otherwise it's already there and stops saying `(coming soon)` on its own. `Category` here must exactly match the folder name from step 1. Then add a row to the table above and bump the count badge, since neither updates itself.
