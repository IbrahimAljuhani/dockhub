...
Model names are `provider/model` — the schema's own example is `ollama/qwen3:8b`, so a pulled `llama3.1:8b` becomes **`ollama/llama3.1:8b`**. Afterwards, `docker compose restart openclaw` and check the startup line reads `agent model: ollama/…` rather than `openai/gpt-5.5`.
