...
# this service's volume holds LocalAI's backends and its All-In-One model set.
# That is gigabytes of already-compressed binary, gzip cannot shrink it, and
# none of it is yours — to get it back you:
#   restart it — LocalAI re-fetches on first start
...
backup_localai() {
    backup_service_config_only "localai" "$1" "$2"
}
