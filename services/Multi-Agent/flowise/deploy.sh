...
if [[ -f "$RUNTIME_DIR/docker-compose.yml" ]]; then
    ...
    # ── The model-provider network, and this one MUST self-heal ──────────
    # The override written below joins 'models-net' unconditionally, and the
    # network is DECLARED in the base compose. A kept compose file from before
    # that change declares no such network, so the merged project would
    # reference an undefined one: `compose config -q` rejects it and this
    ...
    if ! grep -qE '^[[:space:]]+models-net:' "$RUNTIME_DIR/docker-compose.yml"; then
        ...
        cp "$SOURCE_DIR/docker-compose.yml" "$RUNTIME_DIR/docker-compose.yml"
        print_info "Moved this deployment to 'models-net'. It reaches an Ollama / llama.cpp /"
        print_info "LocalAI by container name (http://ollama:11434) exactly as before — but is"
        print_info "no longer on the same network as OpenHands, which has no authentication"
        print_info "and holds the Docker socket. Previous compose kept as"
        print_info "    docker-compose.yml.pre-modelsnet"
    fi
...
if [[ ! -f "$RUNTIME_DIR/.dockhub-asked" ]]; then
    prompt_agent_network "Flowise" "models-net"
